...
class date_format(tornado.web.UIModule):
    
    def render(self, arg_time):
        
        atime = time.localtime(arg_time)
        
        return time.strftime('%Y-%m-%d %H:%M:%S',atime)
    
    
class socket_type_format(tornado.web.UIModule):
    
    def render(self, socket_type):
        
        type_list = {1:'游戏操作',2:'服务器状态',3:'获取区服信息'}
        
        return type_list[socket_type]
